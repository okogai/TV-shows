...
export const fetchSearchResults = createAsyncThunk <ShowOption[], string>('tvShows/createAsyncThunk', async (title: string) => {
  const response = await axiosAPI<ShowSearchResult[]>(`search/shows?q=${title}`);
...
export const fetchTVShowDetails = createAsyncThunk<ITVShow, number>('tvShows/fetchTVShowDetails', async (id: number) => {
